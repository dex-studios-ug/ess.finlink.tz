;(function (module) {
    mifosX.controllers = _.extend(module, {
        ViewLoanDetailsControllerx: async  function (rootScope,   scope, routeParams, resourceFactory,paginatorService, location, route, http, $uibModal, dateFilter, API_VERSION, $sce, $rootScope) {
            scope.loandocuments = [];
            scope.report = false;
            ...
            scope.clickEvent = async function (eventName, accountId) {
                eventName = eventName || "";
                switch (eventName) {
                    case "addloancharge":
                    ...
                    case "approve":
                    if(!scope.isApprover){
                        return alert('Youre not Configured as an Approver')
                    }else if(scope.noop){
                        var last=[...(scope.approvals||[])].pop();
                       if(scope.approval_endpoint&&scope.userId&&last&&(last.user_id==scope.userId))
                        { const undo = confirm('You have Already Approved this Loan.\n Do you want to undo this Action?')
                        if(undo ){
                            await http.get(scope.approval_endpoint+'/unapprovestep',{params:{
                                user_id:scope.userId,loan_id:accountId
                            }}).then(()=>alert('success'))
                        }
                    }
                        return;
            ...
            resourceFactory.LoanAccountResource.getLoanAccountDetails({loanId: routeParams.id, associations: 'all',exclude: 'guarantors,futureSchedule'},async function (data) {
                ...
                if (data.status.value == "Submitted and pending approval") {
                    ...
                    scope.approval_endpoint=endpoint;
                    scope.userId=userId;
                    
                    const response = await http.get(endpoint,{params:{loan_id: routeParams.id}});
                   if(!response||!response.data||!response.approvers?.length){
                    ...
                    if(response.data){
                        ...
                        scope.isApprover = approvers.filter(a=>a.user_id==userId).length
                        if(!scope.isApprover)scope.noop=1;
                            ...
                            else
                       {
                                     ...
                                     if(approvals.filter(a=>a.user_id==userId).length){
                                        scope.label='Approved'
                                        scope.noop=1;
                                     }
                                ...
                                if(next){
                                  ...
                                  if(next.user_id==userId){
                                    approval_state="Pending Your Approval";
                                     ...
                                     if(approvals.filter(a=>a.user_id==userId).length){
                                        scope.label="Approved"
                                        if(last.user_id==userId)scope.label='Un-Approve'
                                     }
                                  }
                                 // alert(approval_state)
                                }
                            }
                            }
                        }
                        scope.state =approval_state;
    ...
    $controllerProvider.register('ViewLoanDetailsController', [ '$rootScope', '$scope', '$routeParams', 'ResourceFactory','PaginatorService',
         '$location', '$route', '$http', '$uibModal', 'dateFilter', 'API_VERSION', '$sce', '$rootScope', mifosX.controllers.ViewLoanDetailsControllerx])
