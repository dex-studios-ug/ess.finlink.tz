...
// Add to your ess_router

// GET /api/ess/logs - Get paginated ESS logs
...
// GET /api/ess/stats - Get statistics
...
// POST /api/ess/logs/:id/resend - Resend message to ESS
app.post('/logs/:id/resend', async (req, res) => {
    try {
        ...
        // Resend logic here
        const result = await notificationService.sendNotificationRaw(
            log.message_type,
            log.request_payload,
            log.message_id
        );
...
// DELETE /api/ess/logs/:id - Delete log
app.delete('/logs/:id', async (req, res) => {
    try {
        await db.run('DELETE FROM ess_logs WHERE id = ?', [req.params.id]);
...
// POST /api/ess/logs/bulk-delete - Bulk delete logs
app.post('/logs/bulk-delete', async (req, res) => {
    try {
        const { ids } = req.body;
        const placeholders = ids.map(() => '?').join(',');
        await db.run(`DELETE FROM ess_logs WHERE id IN (${placeholders})`, ids);
        res.json({ success: true });
    } catch (error) {
        res.status(500).json({ error: error.message });
    }
...
// GET /api/ess/export - Export to CSV
app.get('/export', async (req, res) => {
    try {
        const { search, messageType, direction, status, dateFrom, dateTo } = req.query;
        let query = 'SELECT * FROM ess_logs WHERE 1=1';
        const params = [];
        
        // Add filters similar to GET /logs endpoint
...
const ess_router =app;
module.exports = { ess_router,app, mifosxClient, signatureService, loanService };
